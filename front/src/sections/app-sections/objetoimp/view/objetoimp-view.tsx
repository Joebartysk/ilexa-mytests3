...
import { TableNoData } from '../../objetoimp/table-no-data';
import { TableEmptyRows } from '../../objetoimp/table-empty-rows';
import { ObjetoImpTableRow } from '../../objetoimp/objetoimp-table-row';
import { ObjetoImpTableHead } from '../../objetoimp/objetoimp-table-head';
import { emptyRows, applyFilter, getComparator } from '../../objetoimp/utils';//Importación de utilidades
import { ObjetoImpTableToolbar } from '../../objetoimp/objetoimp-table-toolbar';

import type { ObjetoImpProps } from '../../objetoimp/objetoimp-table-row';

export function ObjetoImpView() {
  const table = useTable();

  const [filterName, setFilterName] = useState('');

  const [_objetoimp, setObjetoImp] = useState<ObjetoImpProps[]>([]);

  const apiService = new ApiService();

   const [openModal, setOpenModal] = useState(false); //Para agregar
  const [loading, setLoading] = useState(false);
  const [editItem, setEditItem] = useState<ObjetoImpProps | null>(null); //Para editar

  const [openConfirmDialog, setOpenConfirmDialog] = useState(false);//Validacion para antes de eliminar
  const [itemToDelete, setItemToDelete] = useState<ObjetoImpProps | null>(null);//Para eliminar
//===============================================

// Función para cargar datos
  const fetchObjetoImp = async () => {
    try {
      setLoading(true);
      const data = await apiService.getAllObjetoImp();
      setObjetoImp(data);
    } catch (error) {
      console.error('Error al recuperar lista de Objeto Impuestos:', error);
  ...
  const handleModalSuccess = async (newItem?: ObjetoImpProps) => {
    try {
      if (newItem) {
        // Si el modal devuelve el nuevo item, agregarlo directamente
        setObjetoImp(prev => [...prev, newItem]);
      } else {
        // Si no, refrescar toda la lista
        await fetchObjetoImp();
      }
      setOpenModal(false);
    } catch (error) {
  ...
  const handleItemUpdate = async (updatedItem: ObjetoImpProps) => {
    try {
      setObjetoImp(prev => 
  ...
  const handleItemDelete = async (deletedId: string) => {
    try {
      await apiService.deleteCatalogItem('objetoimp', deletedId);
      setObjetoImp(prev => prev.filter(item => item.id !== deletedId));
    } catch (error) {
  ...
  const dataFiltered: ObjetoImpProps[] = applyFilter({
    inputData: _objetoimp,
  ...
  return (
    <DashboardContent maxWidth={false} >
      ...
      >
        ...
        <ExportMenu filename="c_ObjetoImp" schema="cfdi_system" table="c_ObjetoImp" />
     ...
     <Card>
          ...
          <TableContainer component={Paper} sx={{ width: '100%', overflowX: 'auto' }} >
            <Table sx={{ borderCollapse: 'collapse',
              ...
              <ObjetoImpTableHead
                ...
                rowCount={_objetoimp.length}
                numSelected={table.selected.length}
                onSort={table.onSort}
                onSelectAllRows={(checked) =>
                  table.onSelectAllRows(
                    checked,
                    _objetoimp.map((objetoimp) => objetoimp.id)
                  )
                }
                headLabel={[
                  { id: 'id', label: 'ID', align: 'center' },
                  { id: 'tax_object_code', label: 'Código Impuesto', align: 'center' },
              ...
              <TableBody>
                ...
                <TableEmptyRows
                  height={30}
                  emptyRows={emptyRows(table.page, table.rowsPerPage, _objetoimp.length)}
                />
      ...
      <CatalogModal
        ...
        catalogKey="objetoimp"
        title={editItem ? 'Editar Objeto Impuestos' : 'Agregar Objeto Impuestos'}
