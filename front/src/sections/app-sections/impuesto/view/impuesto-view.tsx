...
import { TableNoData } from '../../impuesto/table-no-data';
import { TableEmptyRows } from '../../impuesto/table-empty-rows';
import { ImpuestoTableRow } from '../../impuesto/impuesto-table-row';
import { ImpuestoTableHead } from '../../impuesto/impuesto-table-head';
import { emptyRows, applyFilter, getComparator } from '../../impuesto/utils';//Importación de utilidades
import { ImpuestoTableToolbar } from '../../impuesto/impuesto-table-toolbar';

import type { ImpuestoProps } from '../../impuesto/impuesto-table-row';

export function ImpuestoView() {
  const table = useTable();

  const [filterName, setFilterName] = useState('');

  const [_impuesto, setImpuestos] = useState<ImpuestoProps[]>([]);

  const apiService = new ApiService();

 const [openModal, setOpenModal] = useState(false); //Para agregar
  const [loading, setLoading] = useState(false);
  const [editItem, setEditItem] = useState<ImpuestoProps | null>(null); //Para editar

  const [openConfirmDialog, setOpenConfirmDialog] = useState(false);//Validacion para antes de eliminar
  const [itemToDelete, setItemToDelete] = useState<ImpuestoProps | null>(null);//Para eliminar
//===============================================
...
// Función para cargar datos
  const fetchImpuesto = async () => {
    try {
      setLoading(true);
      const data = await apiService.getAllImpuesto();
      setImpuestos(data);
    } catch (error) {
      console.error('Error al recuperar lista de Impuesto:', error);
  ...
  const handleModalSuccess = async (newItem?: ImpuestoProps) => {
    try {
      if (newItem) {
        // Si el modal devuelve el nuevo item, agregarlo directamente
        setImpuestos(prev => [...prev, newItem]);
      } else {
  ...
  const handleItemUpdate = async (updatedItem: ImpuestoProps) => {
    try {
      setImpuestos(prev => 
  ...
  const handleItemDelete = async (deletedId: string) => {
    try {
      await apiService.deleteCatalogItem('impuesto', deletedId);
      setImpuestos(prev => prev.filter(item => item.id !== deletedId));
    } catch (error) {
  ...
  const dataFiltered: ImpuestoProps[] = applyFilter({
    inputData: _impuesto,
  ...
  return (
    <DashboardContent maxWidth={false} >
      ...
      >
        ...
        <ExportMenu filename="c_Impuesto" schema="cfdi_system" table="c_Impuesto" />
     ...
     <Card>
          ...
          <TableContainer component={Paper} sx={{ width: '100%', overflowX: 'auto' }} >
            <Table sx={{ borderCollapse: 'collapse',
              ...
              <ImpuestoTableHead
                ...
                rowCount={_impuesto.length}
                numSelected={table.selected.length}
                onSort={table.onSort}
                onSelectAllRows={(checked) =>
                  table.onSelectAllRows(
                    checked,
                    _impuesto.map((impuesto) => impuesto.id)
                  )
                }
                headLabel={[
                  { id: 'id', label: 'ID', align: 'center' },
                  { id: 'impuesto', label: 'Impuesto', align: 'center' },
                  { id: 'description', label: 'Descripción', align: 'center' },
                  { id: 'tax_withholding', label: 'Retención Impuestos', align: 'center' },
                  { id: 'tax_transferred', label: 'Impuestos Transferidos', align: 'center' },
                  { id: 'local_federal', label: 'Local Federal', align: 'center' },
              ...
              <TableBody>
                ...
                <TableEmptyRows
                  height={30}
                  emptyRows={emptyRows(table.page, table.rowsPerPage, _impuesto.length)}
                />
      ...
      <CatalogModal
        ...
        catalogKey="impuesto"
        title={editItem ? 'Editar Impuesto' : 'Agregar Impuesto'}
