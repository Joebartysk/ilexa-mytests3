...
import { TableNoData } from '../../moneda/table-no-data';
import { TableEmptyRows } from '../../moneda/table-empty-rows';
import { MonedaTableRow } from '../../moneda/moneda-table-row';
import { MonedaTableHead } from '../../moneda/moneda-table-head';
import { MonedaTableToolbar } from '../../moneda/moneda-table-toolbar';
import { emptyRows, applyFilter, getComparator } from '../../moneda/utils';//Importación de utilidades

import type { MonedaProps } from '../../moneda/moneda-table-row';

export function MonedaView() {
  const table = useTable();

  const [filterName, setFilterName] = useState('');

  const [_moneda, setMoneda] = useState<MonedaProps[]>([]);

  const apiService = new ApiService();

 const [openModal, setOpenModal] = useState(false); //Para agregar
  const [loading, setLoading] = useState(false);
  const [editItem, setEditItem] = useState<MonedaProps | null>(null); //Para editar

  const [openConfirmDialog, setOpenConfirmDialog] = useState(false);//Validacion para antes de eliminar
  const [itemToDelete, setItemToDelete] = useState<MonedaProps | null>(null);//Para eliminar
//===============================================
...
// Función para cargar datos
  const fetchMoneda = async () => {
    try {
      setLoading(true);
      const data = await apiService.getAllMoneda();
      setMoneda(data);
    } catch (error) {
      console.error('Error al recuperar lista de Moneda:', error);
  ...
  const handleModalSuccess = async (newItem?: MonedaProps) => {
    try {
      if (newItem) {
        // Si el modal devuelve el nuevo item, agregarlo directamente
        setMoneda(prev => [...prev, newItem]);
      } else {
  ...
  const handleItemUpdate = async (updatedItem: MonedaProps) => {
    try {
      setMoneda(prev => 
  ...
  const handleItemDelete = async (deletedId: string) => {
    try {
      await apiService.deleteCatalogItem('moneda', deletedId);
      setMoneda(prev => prev.filter(item => item.id !== deletedId));
    } catch (error) {
  ...
  const dataFiltered: MonedaProps[] = applyFilter({
    inputData: _moneda,
  ...
  return (
    <DashboardContent maxWidth={false} >
      ...
      >
        ...
        <ExportMenu filename="c_Moneda" schema="cfdi_system" table="c_Moneda" />
     ...
     <Card>
          ...
          <TableContainer component={Paper} sx={{ width: '100%', overflowX: 'auto' }} >
            <Table sx={{ borderCollapse: 'collapse',
              ...
              <MonedaTableHead
                ...
                rowCount={_moneda.length}
                numSelected={table.selected.length}
                onSort={table.onSort}
                onSelectAllRows={(checked) =>
                  table.onSelectAllRows(
                    checked,
                    _moneda.map((moneda) => moneda.id)
                  )
                }
                headLabel={[
                  { id: 'id', label: 'ID', align: 'center' },
                  { id: 'currency_code', label: 'Moneda', align: 'center' },
                  { id: 'description', label: 'Descripción', align: 'center' },
                  { id: 'decimal_places', label: 'Decimales', align: 'center' },
                  { id: 'variation_percentage', label: 'Porcentaje Variación', align: 'center' },
              ...
              <TableBody>
                ...
                <TableEmptyRows
                  height={30}
                  emptyRows={emptyRows(table.page, table.rowsPerPage, _moneda.length)}
                />
      ...
      <CatalogModal
        ...
        catalogKey="moneda"
        title={editItem ? 'Editar Moneda' : 'Agregar Moneda'}
