...
export type RegimenFiscalProps = {
  id: string,
...
type RegimenFiscalTableRowProps = {
  row: RegimenFiscalProps;
  selected: boolean;
  onSelectRow: () => void;
...
export function RegimenFiscalTableRow({
  row,
  selected,
  onSelectRow,
  onEdit,
  onDelete,
}: RegimenFiscalTableRowProps) {
  ...
  return (
    <>
      <TableRow hover tabIndex={-1} role="checkbox" selected={selected}>
        ...
        <TableCell size="small" sx={{ width: '20%' }} >{row.tax_regime_code}</TableCell>

        <TableCell size="small" sx={{ width: '30%' }} >{row.description}</TableCell>

        <TableCell size="small" sx={{ width: '10%' }} >{row.fisica}</TableCell>

        <TableCell size="small" sx={{ width: '10%' }} >{row.moral}</TableCell>
